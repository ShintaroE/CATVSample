...
interface NWEquipmentProps {
  data: AdditionalCosts['nwEquipment']
  onChange: (data: AdditionalCosts['nwEquipment']) => void
  isOpen: boolean
  onToggle: () => void
}

export default function NWEquipment({
  data,
  onChange,
  isOpen,
  onToggle,
  ...
  return (
    <div className="bg-white p-3 rounded border">
      ...
      >
        <h5 className="text-sm font-medium text-gray-900">NWスパハンなど</h5>
      ...
      {isOpen && (
        <div className="mt-3 grid grid-cols-3 gap-4 text-sm">
          ...
          <div>
            ...
            <input
              type="date"
              value={data.quotationCreatedDate || ''}
              onChange={(e) =>
                onChange({ ...data, quotationCreatedDate: e.target.value })
              }
          ...
          <div>
            ...
            <input
              type="date"
              value={data.quotationSubmittedDate || ''}
              onChange={(e) =>
                onChange({ ...data, quotationSubmittedDate: e.target.value })
              }
              disabled={isFieldDisabled}
              className={`w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                isFieldDisabled
